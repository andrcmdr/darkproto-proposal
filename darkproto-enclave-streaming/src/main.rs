...
use darkproto_enclave_streaming::cli_parser::{ServerArgs, ClientArgs, RxTxArgs};
use darkproto_enclave_streaming::config::AppConfig;
use darkproto_enclave_streaming::{client, server, rxtx, txrx};
...
fn main() {
    ...
    match args.subcommand() {
        ...
        Some(("enclave-mode", args)) => {
            let rxtx_args = RxTxArgs::new_with(args).unwrap();
            rxtx(rxtx_args, app_config).unwrap();
        }
        Some(("host-mode", args)) => {
            let rxtx_args = RxTxArgs::new_with(args).unwrap();
            txrx(rxtx_args, app_config).unwrap();
        }
        Some(_) | None => ()
    }
}
